...
구분자와 띄어쓰기 변형(`880101 - 1234567`), 전각 표기(`８８０１０１`), 자모 분리형 한글을
정규화 층에서 흡수한 뒤 탐지하고, 찾은 구간을 **원문 좌표로 정확히 되돌린다.**
...
합성 코퍼스 2,400건(양성 1,920 / 음성 480)에 대한 실측이다. 씨앗이 고정이라 누구든 같은 수를 얻는다.

| 축 | 값 |
| --- | --- |
| 전체 재현율 | **99.8%** (TP 1916 / FN 4) |
| 전체 정밀도 | **99.4%** (FP 12) |
| 마스킹 원문 복원 실패 | **0건** (2,400건 전량 왕복) |
| 음성 표본 오탐률 | 1.7% (480건 중 8건) |

표기 변형별로는 한글 수사·전각·공백이 100%, 숫자만과 하이픈이 99.6%다. 엔티티별로는 계좌번호가
95.0%, 나머지 여덟 종이 100%다. 재현 방법은 [16. 빌드와 테스트](#16-빌드와-테스트)에 있다.
...
| 정규화 (`normalize`) | 자모 조합, 전각 폴딩, 한글 수사 역변환, 구분자 흡수 |
...
| 특징 | 의미 |
| --- | --- |
| **변형 표기 인식** | 한글 수사 표기, 구분자와 띄어쓰기 변형, 전각 표기, 자모 분리형 한글을 정규화 층에서 흡수한다 |
...
pub struct Rejection {
    pub entity: EntityKind,
    pub source: Span,
    pub reason: RejectReason,  // ChecksumFailed | NoContext | BelowThreshold | Outranked
    pub score: f32,
}
```

`EntityKind` 는 `Resident`, `ForeignerRegistration`, `BusinessRegistration`, `CreditCard`,
`BankAccount`, `Phone`, `Email`, `DriverLicense`, `BirthDate` 아홉 종이다. `label()` 이 한국어
이름을 낸다.
...
| 4 | `separator` | 숫자 사이의 붙임표, 점, 공백, 괄호를 흡수한다 | `separator.hyphen`, `separator.whitespace` 외 |
...
| 유사문자 교정 | `O` 와 `0`, `l` 과 `1` 을 바꾸지 않는다 | `POLO` 가 `P0L0` 이 된다. 없던 숫자열을 만든다 |
...
계좌번호, 운전면허번호, 생년월일은 숫자열만으로는 아무것도 말할 수 없다. `1234567890` 이 계좌번호인지
주문번호인지는 규칙으로 알 수 없다. 이 셋은 문맥 총점이 `min_context`(기본 0.3) 미만이면 후보 자체가
`NoContext` 로 떨어진다.
...
- 엔티티 9종의 값. 검증식이 있는 것은 통과하는 값으로 만든다
- 같은 값의 표기 변형 6종: 숫자만, 하이픈, 공백, 전각, 한글 수사, 원형
- 문맥 단서가 있는 문장과 없는 문장
- 오탐을 유도하는 음성 표본 7종: 주문번호, 송장번호, 운송장번호, 제품 코드, 금액, 회원번호,
  그리고 수사 음절로 이루어진 일상어(`이사 갑니다`, `사구 팔구`)

**음성 표본을 통과하기 쉽게 고르지 않았다.** 13자리 운송장번호가 우연히 Luhn 을 통과하면 그것은
진짜 오탐이고, 그 확률까지 수치에 반영되는 것이 맞다. 실제로 남은 오탐 12건 중 7건이 이것이다.
...
계좌번호·운전면허번호·생년월일은 문맥 없이는 판정하지 않기로 설계돼 있다. 그런 표본은 정답이
모호하다. 텍스트에는 개인정보가 들어 있지만 라이브러리는 설계상 그것을 내지 않고, 그 판단은
...
| `Config` | `min_score`, `min_context`, `context_window`, `nfc`, `fold`, `hangul`, `separator`, `numeral_*`, `weights`, `set_weights(...)` |
...
| 여권번호 | 영숫자 혼합이라 별도 스캐너가 필요하다. 문맥 단서 사전은 준비돼 있다 |
...
**우연한 검증식 통과는 막을 수 없다.** 무작위 13자리의 약 10퍼센트가 Luhn 을 통과한다. 그런
운송장번호는 카드번호로 걸린다.
